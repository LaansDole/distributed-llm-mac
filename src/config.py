...
def get_config_from_env() -> LoadBalancerConfig:
    ...
    return LoadBalancerConfig(
        health_check_interval=int(
            os.getenv("HEALTH_CHECK_INTERVAL", DEFAULT_CONFIG.health_check_interval)
        ),
        ...
        max_concurrent_batch=int(
            os.getenv("MAX_CONCURRENT_BATCH", DEFAULT_CONFIG.max_concurrent_batch)
        ),
        connection_pool_size=int(
            os.getenv("CONNECTION_POOL_SIZE", DEFAULT_CONFIG.connection_pool_size)
        ),
        dns_cache_ttl=int(os.getenv("DNS_CACHE_TTL", DEFAULT_CONFIG.dns_cache_ttl)),
        enable_metrics=os.getenv("ENABLE_METRICS", str(DEFAULT_CONFIG.enable_metrics)).lower()
        == "true",
