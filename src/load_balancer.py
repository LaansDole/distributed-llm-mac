...
Supports both Ollama and LM Studio workers
"""
...
class LoadBalancer:
    """Load balancer for distributing LLM inference requests"""
    ...
    async def process_request(self, prompt: str, **kwargs) -> Dict[str, Any]:
        ...
        raise Exception(f"Request failed after {self.config.max_retries + 1} attempts. Last error: {last_error}")
    ...
    def print_status(self):
        ...
        print("-" * 80)
        print(f"{'ID':<20} {'HOST':<15} {'PORT':<6} {'TYPE':<11} {'LOAD':<6} {'HEALTH':<8} {'RESP_TIME':<10}")
        print("-" * 80)

        for worker in self.workers:
            health = "✓" if worker.is_healthy else "✗"
            load_pct = f"{worker.load_percentage:.0f}%"
            resp_time = f"{worker.average_response_time:.2f}s" if worker.average_response_time > 0 else "N/A"

            print(f"{worker.id:<20} {worker.host:<15} {worker.port:<6} "
